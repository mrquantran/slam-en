...
// Algebra operations of dense matrices (inverse, eigenvalues, etc.)
#include <Eigen/Dense>
...
/****************************
 * This program demonstrates the use of the Eigen basic matrix class
 * ****************************/
int main(int argc, char **argv) {
    // All vectors and matrices in Eigen are Eigen::Matrix, which is a template
    // class. Its first three parameters are: data type, row, column Declaration
    // of a 2*3 float matrix
    Matrix<float, 2, 3> matrix_23;

    // At the same time, Eigen provides many built-in types through typedef, but
    // the bottom layer is still Eigen::Matrix For example, Vector3d is actually
    ...
    // If you are not sure about the size of the matrix, you can use a dynamic
    // size matrix
    ...
    // There are many other types like this, we will not list them one by one

    // Here is the operation on the Eigen matrix
    ...
    // In Eigen, you cannot mix two different types of matrices like this is
    ...
    // Try canceling the comment below to see what Eigen will report
    // Eigen::Matrix<double, 2, 3> result_wrong_dimension =
    // matrix_23.cast<double>() * v_3d;

    // Some matrix operations
    // The basic operations are not demonstrated, just use +-*/ directly.
    ...
    // Usually use matrix decomposition to solve, such as QR decomposition, the
    ...
    // For Positive definite matrix, you can also use cholesky decomposition to
    // solve equations
    time_stt = clock();
